...
export function ActiveWars({ isLoaded }: ActiveWarsProps) {
  const activeWars = [
    {
      id: "war-1",
      clan1: { name: "CYBER WOLVES", logo: "/placeholder.svg?height=100&width=100" },
      clan2: { name: "NEON KNIGHTS", logo: "/placeholder.svg?height=100&width=100" },
      timeRemaining: "2d 14h",
      score: { clan1: 1240, clan2: 1180 },
    },
    {
      id: "war-2",
      clan1: { name: "PIXEL PUNKS", logo: "/placeholder.svg?height=100&width=100" },
      clan2: { name: "DEFI DEMONS", logo: "/placeholder.svg?height=100&width=100" },
      timeRemaining: "1d 6h",
      score: { clan1: 980, clan2: 1050 },
    },
    {
      id: "war-3",
      clan1: { name: "META MARINES", logo: "/placeholder.svg?height=100&width=100" },
      clan2: { name: "CHAIN CHAMPIONS", logo: "/placeholder.svg?height=100&width=100" },
      timeRemaining: "3d 8h",
    ...
    >
      <div className="bg-[#15151B] rounded-lg p-6 h-full border border-[#1F1F2A]">
        <div className="flex items-center justify-between mb-6">
          ...
          <Button asChild variant="link" className="text-[#C300FF] p-0 h-auto font-medium">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-6">
                  <div className="flex flex-col items-center">
                    ...
                    />
                    <span className="text-[#B1B1B1] text-xs mt-1">{war.clan1.name}</span>
                    <span className="text-[#39FF14] font-bold">{war.score.clan1}</span>
                  </div>

                  <div className="text-center">
                    <div className="text-[#666666] text-xs">VS</div>
                    <div className="text-[#FAFAFA] text-xs mt-1">{war.timeRemaining}</div>
                  ...
                  <div className="flex flex-col items-center">
                    <Image
                      ...
                      className="rounded-full border-2 border-[#39FF14]"
                    />
                    <span className="text-[#B1B1B1] text-xs mt-1">{war.clan2.name}</span>
                    <span className="text-[#39FF14] font-bold">{war.score.clan2}</span>
                  </div>
                </div>
